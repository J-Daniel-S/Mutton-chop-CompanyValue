...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		CompanyController.setReturnOnInvestment("10");
		...
		exitProcedure();
		setVisible(true);
		// this method contains the timed refresher
		refresh();

		if (JSONConverter.getPath().toFile().exists()) {
			try {
				CompanyController.setStocks(JSONConverter.convertFromJSON(JSONConverter.getPath()));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		if (!CompanyController.getStocks().isEmpty()) {
			CompanyController.resynch(this);
		}
	...
	private void exitProcedure() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				if (!CompanyController.getStocks().isEmpty()) {
					JSONConverter.convertToJSON(CompanyController.getStocksList(), JSONConverter.getPath());
				}
				System.exit(0);
			}
		});
...
//	for use later
	...
	private void popupClicked(MainFrame mainFrame) {
...
//		query JMenuItem
		queryListenerSetup(mainFrame);

//		check values JMenuItem
		valuesListenerSetupd();

//		autoUpdate JMenuItem
...
//		resynch JMenuItem
	...
	private void queryListenerSetup(MainFrame mainFrame) {
		TableController.setListener(infoTablePanel, (QueryEvent e) -> {
			mainFrame.addToTextPanel(e.getText());
		});
		TableController.setListener(finTablePanel, (QueryEvent e) -> {
			mainFrame.addToTextPanel(e.getText());
		});
		TableController.setListener(retTablePanel, (QueryEvent e) -> {
			mainFrame.addToTextPanel(e.getText());
		});
	}

	private void resynchMenuSetup() {
		TableController.setListener(infoTablePanel, (SynchEvent e) -> {
			CompanyController.resynch(e.getRow(), MainFrame.this);
			TableController.refresh(infoTablePanel, finTablePanel, retTablePanel);
		});
		TableController.setListener(finTablePanel, (SynchEvent e) -> {
			CompanyController.resynch(e.getRow(), MainFrame.this);
			TableController.refresh(infoTablePanel, finTablePanel, retTablePanel);
		});
		TableController.setListener(retTablePanel, (SynchEvent e) -> {
			CompanyController.resynch(e.getRow(), MainFrame.this);
			TableController.refresh(infoTablePanel, finTablePanel, retTablePanel);
		});
	}

	private void autoUpdateListenerSetup() {
		TableController.setListener(infoTablePanel, (UpdateEvent e) -> {
			if (e.getText().equals("autoUpdate")) {
				CompanyController.setAutoUpdate(e.getRow(),
						CompanyController.getStocks().get(e.getRow()).isAutoUpdate());
			}
			infoTablePanel.refresh();
		});
		TableController.setListener(finTablePanel, (UpdateEvent e) -> {
			if (e.getText().equals("autoUpdate")) {
				CompanyController.setAutoUpdate(e.getRow(),
						CompanyController.getStocks().get(e.getRow()).isAutoUpdate());
			}
			finTablePanel.refresh();
		});
		TableController.setListener(retTablePanel, (UpdateEvent e) -> {
			if (e.getText().equals("autoUpdate")) {
				CompanyController.setAutoUpdate(e.getRow(),
						CompanyController.getStocks().get(e.getRow()).isAutoUpdate());
			}
			retTablePanel.refresh();
		});
	}

	private void valuesListenerSetupd() {
	...
	private void removeListenerSetup() {
	...
	private void manualEntryListenerSetup() {
	...
	private void handleMultiEntryPanel(ClickEvent e) {
		MultiEntryPanel multi = new MultiEntryPanel(e.getRow(), this);
		HashMap<String, JTextField[]> input = DataEntryHandler.multiEntry(e.getColumnName(), e.getRow(), MainFrame.this,
				multi);
		if (!input.isEmpty()) {
			try {
				HashMap<String, List<String>> output = getInputValues(input, e.getRow());
				CompanyController.manualValues(e.getRow(), output, this);
			} catch (NullPointerException e2) {
				textPanel.addText(
						"Error in data entered.  Check format and try again\nPlease do not leave blank spaces");
				e2.printStackTrace();
...
//	generates the event that results in the stock being added to the table
	...
	private void buildCalcMenu(JMenu calculate) {
		...
		explain.addActionListener((ActionEvent e) -> {
			textPanel.addText(Descriptions.whyCroicOrCashFlow);
		});
	...
	private void buildSortMenu(JMenu sort) {
		...
		callSortByMethods(ticker, price, sector, industry, mcap, croic, prof, avgProf, roa, roe, fcf, avgfcf, coh, eqdt,
				cohdt, yield, fairValue);
	...
	private void callSortByMethods(JMenuItem ticker, JMenuItem price, JMenuItem sector, JMenuItem industry,
			JMenuItem mcap, JMenuItem croic, JMenuItem prof, JMenuItem avgProf, JMenuItem roa, JMenuItem roe,
			JMenuItem fcf, JMenuItem avgfcf, JMenuItem coh, JMenuItem eqdt, JMenuItem cohdt, JMenuItem yield,
			JMenuItem fairValue) {
	...
	public void cannotParse(String ticker) {
		addToTextPanel("Cannot parse data entered for " + ticker);
	}
